...
void graphDestructor(Graph* graph)
{
    for (int i = 0; i<graph->vertexNum; i++)
    {
        ...
        adjListNode* nextNode = graph->connectedCities[i].head;
        while (nextNode != NULL)
        {
            currentNode = nextNode;
            nextNode = currentNode->next;
            adjListNodeDestructor(currentNode);
        }
    }
    free(graph);
}
...
void addEdgeAdjList(Graph* graph, int src, int dest, int energy, char* isDirected)
{
    ...
    else if (strcmp(isDirected, "dir") == 0)
    {
        ...
        addEdgeAdjListDirected(graph, src, dest, energy);
    }
    else if (strcmp(isDirected, "revdir") == 0)
    {
        /* Adds edges to be directed from dest to src
        Utilises the same function but swaps round the 2nd and 3rd parameters
        so the direction is reversed */
        addEdgeAdjListDirected(graph, dest, src, energy);
    }
    else
    {
...
int bellmanford(Graph* graph, int startSrc, int pathDestination)
{
    ...
    //Firsly fill the distance array and predecessor array
    ...
    //Run the algorithm to find the shortest path by relaxing the edges |V| - 1 times
    ...
    //Used to detect any negative cycle
    //If any edge value decreases then we have a negative cycle in the graph
    //Hence, we cannot find the shortest distances
    for (j = 0; j<graph->vertexNum; j++)
    {
        nextNode = graph->connectedCities[j].head;
        if (nextNode == NULL)
    ...
    //Prints out the number of negative cycles found in the graph
    //If none found nothing is printed
    ...
    printf("Distance array from source %s: ", returnCity(startSrc));
    displayResult(distance, graph->vertexNum);
    printf("Predecessor array: ");
    displayResult(predecessor, graph->vertexNum);
    printf("\n");
